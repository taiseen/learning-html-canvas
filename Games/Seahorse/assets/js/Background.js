import BgLayer from "./BgLayer.js";

// Put all Layer Objects together to animate all game world
class Background {
    constructor(game) {
        this.game = game;

        this.img1 = document.getElementById('layer1');
        this.img2 = document.getElementById('layer2');
        this.img3 = document.getElementById('layer3');
        this.img4 = document.getElementById('layer4');
        
        this.layer1 = new BgLayer(this.game, this.img1, .3);
        this.layer2 = new BgLayer(this.game, this.img2, .6);
        this.layer3 = new BgLayer(this.game, this.img3, .8);
        this.layer4 = new BgLayer(this.game, this.img4, 1);

        this.layers = [this.layer1, this.layer2, this.layer3, this.layer4];
    }

    draw(context) {
        this.layers.forEach(layer => layer.draw(context));
    }

    update() {
        this.layers.forEach(layer => layer.update());
    }
}

export default Background;
